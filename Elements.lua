...
function Elements:Init(Library, Toggles, Options, ScreenGui, BaseAddons)
	...
	function Funcs:AddToggle(Idx, Info)
		...
		-- Switch moderno (40x20 para dar espaço ao knob)
		local SwitchOuter = Library:Create('Frame', {
			BackgroundColor3 = _Color3FromRGB(50, 50, 50),
			BorderSizePixel = 0,
			Size = _UDim2New(0, 40, 0, 20),
			Position = _UDim2New(1, -40, 0.5, -10),
			ZIndex = 6,
			Parent = ToggleFrame,
		})
		Library:Create('UICorner', {
			CornerRadius = _UDimNew(0, 10),
			Parent = SwitchOuter,
		})
		...
		-- Animações com Spring para o switch
		...
		local switchConnection = RunService.RenderStepped:Connect(function()
			...
			-- Interpolar posição do knob (de 2px a 22px)
			local knobX = 2 + (value * 20)
			SwitchKnob.Position = _UDim2New(0, knobX, 0.5, -8)
		end)
		
		-- Hover effect no switch com animação Spring
		...
		local switchHoverConnection = RunService.RenderStepped:Connect(function()
			...
			SwitchStroke.Transparency = 0.3 - (alpha * 0.3)
		end)
		
		SwitchOuter.MouseEnter:Connect(function()
			switchHoverSpring.Target = 1
			switchHoverSpring.Speed = 20
			switchHoverSpring.Damper = 0.7
		end)
		...
		-- Função Display com animação Spring
		function Toggle:Display()
			if Toggle.Value then
				-- Ativado: cor accent (branco), knob à direita
				switchSpring.Target = 1
				switchSpring.Speed = 25
				switchSpring.Damper = 0.75
